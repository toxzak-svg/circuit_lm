"""Benchmark: Bracket-only vocab — PDA vs FSM vs PPM depth generalization

Purpose
-------
Isolate the stack advantage by removing comma (non-structural token).
Brackets only: 6 tokens (OPEN/CLOSE × 3 types) + EOS = 7 tokens.
Predicting the next open/close requires tracking bracket type AND depth.

Hypothesis: Without comma noise, PDA stack benefit should be clearer.

Results
-------
v1 (200 train, 2-8 pairs, 8 tokens): PPM dominates
v2 (400 train, 4-12 pairs, 8 tokens): PPM still wins but PDA beats FSM +3.8pp OOD
v3 (400 train, 4-12 pairs, 7 tokens, bracket-only): TESTING
"""
...
# ---------------------------------------------------------------------------
# Bracket-only vocabulary (7 tokens)
# ---------------------------------------------------------------------------
T_OPEN_PAREN  = 0
T_CLOSE_PAREN = 1
T_OPEN_BRACK  = 2
...
T_CLOSE_BRACE = 5
T_EOS         = 6
VOCAB_SIZE    = 7  # brackets only, no comma
...
def _gen_mixed_bracket_seq(
    ...
    """Generate one mixed-bracket balanced sequence (bracket-only vocab)."""
...
def gen_test_seqs_per_depth(target_depth: int, num_seqs: int, rng: random.Random, min_pairs: int = 4, max_pairs: int = 12) -> list[list[int]]:
    seqs = []
    for _ in range(num_seqs * 3):
...
def main():
    parser = argparse.ArgumentParser(description="Bracket-only vocab benchmark (v3)")
    ...
    print(f"=== Multi-bracket benchmark v3 (bracket-only vocab, {VOCAB_SIZE} tokens) ===")
    ...
    for depth in TEST_DEPTHS:
        ...
        results.append({"depth": depth, "pda_acc": pda_acc, "fsm_acc": fsm_acc, "ppm_acc": ppm_acc, "ood": depth > args.max_train_depth})
    ...
    if ood_results:
        ...
        print(f"OOD avg: PDA={avg_pda:.1f}%, FSM={avg_fsm:.1f}%, PPM={avg_ppm:.1f}%")
        print(f"  PDA vs FSM: {avg_pda - avg_fsm:+.1f}pp | PDA vs PPM: {avg_pda - avg_ppm:+.1f}pp")
    if id_results:
        ...
        print(f"ID avg:  PDA={avg_pda:.1f}%, FSM={avg_fsm:.1f}%, PPM={avg_ppm:.1f}%")

    csv_path = pathlib.Path(__file__).parent.parent / "results" / "benchmark_code_v3_bracket_only.csv"
